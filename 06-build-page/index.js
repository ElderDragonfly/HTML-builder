...
async function createFile(filePath, data) {
  // создаёт файл или очищает его, для перезаписи стилей
  try {
...
async function copyAssetsDirectory(sourceDir, targetDir) {
...
async function generareIndexHTML() {
  try {
    ...
    for (const entry of entries) {
      ...
      sourceHTMLData = await replaceTemplateTagWithComponent(
        sourceHTMLData,
        tag,
      );
    }
